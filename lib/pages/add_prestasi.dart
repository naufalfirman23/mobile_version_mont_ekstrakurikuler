...
class _AddPrestasiPageState extends State<AddPrestasiPage> {
  ...
  Future<void> _pickFile() async {
    try {
      final result = await FilePicker.platform.pickFiles(
        ...
        allowedExtensions: ['jpg', 'jpeg', 'png'], // Ekstensi file
      );
  ...
  Future<void> _submitData() async {
    ...
    try {
      ...
      final message = jsonDecode(responseBody);
      final text = message['message'];

      if (response.statusCode == 200) {
        alertWidget.SuccessAlert(context, "$text");
      } else {
        alertWidget.ErrorAlert(context, "$text");
      }
    } catch (e) {
      print(e);
      ScaffoldMessenger.of(context).showSnackBar(
